...
class Star_euiler:
    ...
    def calculate_potential_energy(self, other_star, softening=1e7):
    ...
    def apply_velocity(self, previous_v, dt):
        #self.p += (previous_v + self.v).div(2)*(dt)
        self.p += self.v * (dt) #- (self.a*(dt**2)).div(2)

    def update(self, stars, dt, previous_v, softening=1e7):
        self.calculate_star_gravitations(stars, softening)
        self.apply_acceleration(dt)
        self.apply_velocity(previous_v, dt)
